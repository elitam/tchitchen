...
export default function HistoriquePage() {
  ...
  // HELPER: Formater YYYY-MM-DD en DD-MM (Simplifié)
  const formatDateDisplay = (dateStr: string) => {
    if (!dateStr || dateStr === 'TOUT') return 'JJ-MM'
    const [, m, d] = dateStr.split('-') // On ignore l'année (index 0)
    return `${d}-${m}`
  }
      ...
      <div className="space-y-6 mb-10">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar items-center">
            ...
            {/* SÉLECTEUR DATE PERSO (Format JJ-MM) */}
            <div className={`relative flex items-center px-4 py-2 rounded-full border transition-all ${
              ...
              ? 'bg-white text-black border-white shadow-lg' : 'bg-zinc-900 text-zinc-600 border-zinc-800'
            }`}>
              <span className="text-[10px] font-black uppercase tracking-widest">
                {filterDate.length === 10 ? formatDateDisplay(filterDate) : 'JJ-MM'}
              </span>
              <input 
                type="date" 
