...
export default function Home() {
  ...
  if (error) {
    ...
    setTasks([data[0], ...tasks]) // Ajoute la tâche en haut de la liste
    setNewTaskName('')            // Vide le champ
    setIsModalOpen(false)         // Ferme la fenêtre
  }
  }
  ...
  if (error) setErrorMsg(error.message)
    ...
    setTasks(tasks.filter(t => t.id !== id)) // On l'enlève de l'écran
  }
}
  ...
  <div 
    key={task.id} 
    className={`group w-full flex items-center p-5 border rounded-2xl transition-all ${
  ...
  >
    {/* Zone de texte : Cliquer ici change le statut */}
    <div 
    ...
    {/* Bouton Poubelle : Cliquer ici supprime la tâche */}
    <button 
      onClick={() => {
          ...
          <form onSubmit={addTask}>
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 p-4 rounded-xl font-bold text-zinc-400 hover:text-white transition-colors"
              >
                Annuler
              </button>
              <button 
                type="submit"
                className="flex-1 bg-white text-black p-4 rounded-xl font-bold active:scale-95 transition-transform"
              >
                Ajouter
              </button>
            </div>
