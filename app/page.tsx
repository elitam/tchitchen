...
export default function Home() {
  ...
  longPressActive = true; // On verrouille
  await togglePriority(task.id, task.is_optional, task.display_name);
  if (navigator.vibrate) navigator.vibrate(50);
};

const handleSimpleTap = (task: any) => {
  if (longPressActive) {
    longPressActive = false; // On reset et on annule l'action du tap
    return;
  }
  toggleStatus(task.id, task.status, task.display_name);
};

  // --- NOUVEAU : ÉTAT POUR LES ÉLÉMENTS COCHÉS ---
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // 1. Charger les recettes + Charger les éléments cochés du LocalStorage
  useEffect(() => {
    ...
    // Charger la mémoire du réassort
  ...
  // Sauvegarder dans le localStorage à chaque changement de coche
  useEffect(() => {
 ...
 const togglePriority = async (id: string, currentOptional: boolean, taskName: string) => {
  ...
  {tasks.map((task) => (
    <motion.div 
      ...
      onTap={() => {
        clearTimeout((window as any).lpTimer);
        handleSimpleTap(task);
      }}
      onTapCancel={() => clearTimeout((window as any).lpTimer)}

      // CSS POUR LE SCROLL ET LA SÉLECTION
    ...
    >
      ...
      <button 
        onClick={(e) => {
          e.stopPropagation();
          if(confirm(`Archiver "${task.display_name}" ?`)) deleteTask(task.id, task.display_name);
        }}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-xl border-t border-zinc-900 flex justify-around items-center z-50 pt-3 pb-[calc(env(safe-area-inset-bottom)+10px)] px-6 shadow-lg shadow-black/50">
        <Link href="/" className="flex flex-col items-center gap-1 group">
          <div className={`p-1 transition-all ${pathname === '/' ? 'text-blue-500 scale-110' : 'text-zinc-600'}`}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 13.87A4 4 0 0 1 7.41 6a5.11 5.11 0 0 1 1.05-1.54 5 5 0 0 1 7.08 0A5.11 5.11 0 0 1 16.59 6 4 4 0 0 1 18 13.87V21H6Z"/><line x1="6" y1="17" x2="18" y2="17"/></svg>
          </div>
      ...
      {/* DRAWER SHOPPING LIST (CORRIGÉ POUR COCHER) */}
      <AnimatePresence>
        {isShoppingListOpen && (
          <>
            <motion.div 
                ...
                <div className="space-y-3">
                  {shoppingList.map((ing, i) => {
                    ...
                    return (
                      <div 
                        key={i} 
                        onClick={() => toggleCheck(ing.item, ing.unit)}
                        className={`flex justify-between items-center p-5 rounded-2xl border transition-all active:scale-95 cursor-pointer ${
                          isChecked 
                          ? 'bg-zinc-900 border-zinc-800 opacity-30' 
                          : 'bg-zinc-800 border-zinc-700'
                        }`}
                      >
                        <div className="flex items-center gap-4">
                          {/* PETIT INDICATEUR IOS */}
                          ...
                          <span className={`font-bold uppercase text-sm ${isChecked ? 'line-through text-zinc-500' : 'text-zinc-200'}`}>
                            {ing.item}
                          </span>
                        </div>
